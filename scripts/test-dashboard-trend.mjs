...
test('yesterday and an August date show their complete months when switching day/month/day', () => {
  for (const [selected, displayRange] of [
    ...
    assert.deepEqual(getDashboardTrendPeriod(selected, 'day'), { requestRange: displayRange, displayRange })
...
test('cross-year daily requests include full months while monthly requests retain the selected range', () => {
  ...
  assert.deepEqual(getDashboardTrendPeriod(selected, 'day'), {
    requestRange: { startDate: '2025-12-01', endDate: '2026-01-31' },
...
test('February uses the correct number of days in leap and ordinary years', () => {
  for (const [year, lastDay] of [['2024', '29'], ['2026', '28']]) {
    ...
    assert.deepEqual(getDashboardTrendPeriod(selected, 'day'), {
      requestRange: { startDate: `${year}-02-01`, endDate: `${year}-02-${lastDay}` },
      displayRange: { startDate: `${year}-02-01`, endDate: `${year}-02-${lastDay}` },
    })
  }
})

test('daily requests respect the API range limit when completing calendar months', () => {
  const wholeLeapYear = { startDate: '2024-01-01', endDate: '2024-12-31' }
  assert.deepEqual(getDashboardTrendPeriod({ startDate: '2024-01-15', endDate: '2024-12-15' }, 'day'), {
    requestRange: wholeLeapYear,
    displayRange: wholeLeapYear,
  })

  const selected = { startDate: '2025-12-24', endDate: '2026-12-23' }
  assert.deepEqual(getDashboardTrendPeriod(selected, 'day'), {
    requestRange: selected,
    displayRange: { startDate: '2025-12-01', endDate: '2026-12-31' },
  })
})

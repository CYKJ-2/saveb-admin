...
import InvoiceOperationLogs from './InvoiceOperationLogs.vue'
import '../shared/legacy.css'
const { can, loading, error, run } = useWorkbench('invoice'); const rows = ref<Row[]>([]); const total = ref(0); const filters = reactive({ keyword: '', startDate: '', endDate: '', page: 1, per_page: 20 }); const editor = ref(false); const editingId = ref<number | null>(null); const detailId = ref<number | null>(null); const logsVisible = ref(false)
